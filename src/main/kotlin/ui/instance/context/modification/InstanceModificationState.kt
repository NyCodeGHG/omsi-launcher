...
class InstanceModificationState(parentInstance: Instance? = null) {
    ...
    var name: String by mutableStateOf(parentInstance?.name.takeIf { parentInstance?.isBaseInstance == false } ?: "")
    var path: Path? by mutableStateOf(parentInstance?.directory.takeIf { parentInstance?.isBaseInstance == false })
